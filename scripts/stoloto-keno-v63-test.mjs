// KENO 6.3 · TEST ONLY
// Основа: рабочий arsazet17/pozitron-keno-v72/stoloto-keno-update.mjs.
// Изменения для 6.3: HISTORY_FILE=keno-history-v63.json и полный запрет записи в history.
...
const REPORT_DIR = '.stoloto-v63-test';
...
try {
  ...
  const triple = await readArchiveThreeTimes(page);
  const historyRaw = await readTrustedHistory();
  const validated = validateProduction(triple.stable, historyRaw);
  ...
  const report = {
    ...
    unstableSkipped: triple.mismatches,
    lastTrusted: validated.history.at(-1),
    overlapCount: validated.overlap.length,
    freshCount: validated.fresh.length,
    fresh: validated.fresh,
    latestOfficial: triple.stable.at(-1),
    result: 'PASS',
    historyWasModified: false,
    generatedAt: new Date().toISOString()
  };

  await fs.writeFile(`${REPORT_DIR}/report.json`, JSON.stringify(report, null, 2) + '\n');

  console.log('============================================================');
  console.log('TEST PASS · KENO 6.3 · STOLOTO OAuth');
  console.log(`Три чтения: ${triple.readingCounts.join(' / ')}`);
  console.log(`Стабильных общих строк: ${triple.stable.length}`);
  console.log(`Anchor: №${validated.history.at(-1).draw}`);
  console.log(`Новых подтверждённых: ${validated.fresh.length}`);
  console.log('ВАЖНО: keno-history-v63.json НЕ ИЗМЕНЯЛСЯ.');
  console.log('============================================================');
} catch (error) {
  await fs.writeFile(`${REPORT_DIR}/FAIL.txt`, String(error?.stack || error) + '\n');
  throw error;
} finally {
  await browser.close();
}
